...
def compute_reward_metrics(batch):
    ...
    # 从batch中获取预测值和真实值张量
    if 'pred_ratings' in batch.batch and 'gt_ratings' in batch.batch:
        
        # 只取每个序列的最后一个有效值进行计算
        pred_ratings = batch.batch['pred_ratings'].sum(-1)  # 由于我们只在最后一个位置设置了值，sum 等同于取最后一个非零值
        gt_ratings = batch.batch['gt_ratings'].sum(-1)
        
        # 计算MAE
        mae = torch.mean(torch.abs(pred_ratings - gt_ratings)).item()
        reward_metrics["metrics/mae"] = mae
        
        # 计算RMSE
        mse = torch.mean((pred_ratings - gt_ratings) ** 2)
        rmse = torch.sqrt(mse).item()
        reward_metrics["metrics/rmse"] = rmse
        
        # 计算准确率指标
        exact_match = (pred_ratings == gt_ratings).float().mean().item()
        reward_metrics["metrics/exact_match"] = exact_match
        
        # 计算±1范围内的准确率
        in_one_range = (torch.abs(pred_ratings - gt_ratings) <= 1).float().mean().item()
        reward_metrics["metrics/in_one_range"] = in_one_range
...
class RayPPOTrainer(object):
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            # 收集预测值和真实值张量
            if 'pred_ratings' in test_batch.batch and 'gt_ratings' in test_batch.batch:
                pred_ratings = test_batch.batch['pred_ratings'].sum(-1)  # 只取每个序列的最后一个有效值
                gt_ratings = test_batch.batch['gt_ratings'].sum(-1)
                pred_ratings_lst.append(pred_ratings)
                gt_ratings_lst.append(gt_ratings)
        ...
        # 如果有预测值和真实值，计算相关指标
        if pred_ratings_lst and gt_ratings_lst:
            pred_ratings = torch.cat(pred_ratings_lst, dim=0)
            gt_ratings = torch.cat(gt_ratings_lst, dim=0)
            
            # 计算验证集上的MAE
            val_mae = torch.mean(torch.abs(pred_ratings - gt_ratings)).item()
            metric_dict['val/mae'] = val_mae
            
            # 计算验证集上的RMSE
            val_mse = torch.mean((pred_ratings - gt_ratings) ** 2)
            val_rmse = torch.sqrt(val_mse).item()
            metric_dict['val/rmse'] = val_rmse
            
            # 计算验证集上的准确率指标
            val_exact_match = (pred_ratings == gt_ratings).float().mean().item()
            metric_dict['val/exact_match'] = val_exact_match
            
            # 计算验证集上的±1范围内准确率
            val_in_one_range = (torch.abs(pred_ratings - gt_ratings) <= 1).float().mean().item()
            metric_dict['val/in_one_range'] = val_in_one_range
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('gen', timing_raw):
                        ...
                        print(f"生成时间generate time: {end - start}")
