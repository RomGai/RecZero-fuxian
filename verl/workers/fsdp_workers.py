...
class ActorRolloutRefWorker(Worker):
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        # NOTE: previously a corrupted line '54 = {...}' existed here causing SyntaxError
        # Build meta info with special token ids for generation
        meta_info = {
            'eos_token_id': self.tokenizer.eos_token_id,
            'pad_token_id': self.tokenizer.pad_token_id,
        }
        ...
        with self.rollout_sharding_manager:
            ...
            try:
                ...
                dbg = {
                    ...
                    'prompt_batch_shape': tuple(map(int, prompts.batch['input_ids'].shape)) if 'input_ids' in prompts.batch else None,
                    'prompt_first_ids': prompts.batch['input_ids'][0].tolist()[:32] if 'input_ids' in prompts.batch else None,
                    'rollout_type': type(self.rollout).__name__,
                    'rollout_config': {k: getattr(self.config.rollout, k) for k in getattr(self.config.rollout, 'keys', lambda: [])()},
                    'exception': repr(e),
                }
                fname = f'fsdp_rollout_gen_error_rank{getattr(self, "rank", -1)}.json'
                try:
                    with open(fname, 'w', encoding='utf-8') as f:
                        json.dump(dbg, f, ensure_ascii=False, indent=2)
                    print(f'[FSDPWorker][Error] rollout.generate_sequences failed. Dump saved to {fname}')
                except Exception:
                    traceback.print_exc()
                traceback.print_exc()
                raise
